...
function woocommerce_gateway_gopay_init(){
    ...
    class WC_Gateway_Woo_GoPay extends WC_Payment_Gateway{ 
        ...
        public function __construct(){
            ...
            $callback_url_params = '?wc-api=WC_Gateway_Woo_GoPay&gopay=response';
            $callback_url = get_site_url() .'/'. $callback_url_params;
            $this->filter_callback_url = apply_filters( 'gopay_response_callback_url', $callback_url, $callback_url_params );
        ...
        public function generate_gopay_method_order_html(){
            ...
            $payment_methods = array(
                ...
                'sk_uni'        => __( 'UniCredit Bank', 'woocommerce-gopay' ),
                'sk_sp'         => __( 'Slovenská sporiteľňa', 'woocommerce-gopay' ),
                'sk_vubbank'    => __( 'Všeobecná úverová ', 'woocommerce-gopay' ),
                'sk_tatrabank'  => __( 'Tatra banka', 'woocommerce-gopay' ),
                'sk_pabank'     => __( 'Poštová banka', 'woocommerce-gopay' ),
                'sk_sberbank'   => __( 'Sberbank', 'woocommerce-gopay' ),
                'sk_csob'       => __( 'Československá obchodná banka', 'woocommerce-gopay' ),
                'sk_otpbank'    => __( 'OTP banka ', 'woocommerce-gopay' ),
                'sk_zuno'       => __( 'ZunoPay', 'woocommerce-gopay' ),
            );
        ...
        public function generate_gopay_payment( $order_id, $defaultPaymentChannel ){ 
            ...
            $order = wc_get_order( $order_id );
            $gopay_order_id = $order_id;
            $gopay_order_id =  apply_filters( 'gopay_order_id', $gopay_order_id, $order_id );
            $order_key = Toret_Order_Compatibility::get_order_key( $order );

		
            $payment_method = get_post_meta( $order_id, '_selected_paymentchannel', true );
            $paymentChannels = array( $payment_method );
            if( empty( $defaultPaymentChannel ) ){
                $defaultPaymentChannel = $payment_method;
            }
    
            $billing_data = Toret_Order_Compatibility::get_billing_data( $order );

                if( empty( $billing_data['full_name'] ) ){
                    $gopay_product_name = 'Eshop objednávka';    
                }else{
                    $gopay_product_name = $billing_data['full_name'];
                }

            $gopay_product_name = apply_filters( 'gopay_create_payment_product_name', $gopay_product_name, $order );


            $p1 = $payment_method;
            $p2 = $order_id;
            $p3 = $gopay_product_name;
            $p4 = $gopay_order_id;
            ...
            $curency_lang_data['currency'] = Toret_Order_Compatibility::get_order_currency( $order );
            $order_country = Toret_Order_Compatibility::get_billing_country( $order );
            $country = Toret_GoPay_Define::set_order_country( $order_country );
            $price   = $order->get_total() * 100;

            Toret_GoPay_Log::save( 'Cena objednávky '.$order_id.': '.$price , 'n', 'payment', 'Průběh platby' ); 
    
            //Save order currency
            update_post_meta( $order_id,'gopay_order_currency', $curency_lang_data['currency'] );
            update_post_meta( $order_id,'gopay_order_lang', $curency_lang_data['lang'] );
            update_post_meta( $order_id,'gopay_price', $price ); 
            update_post_meta( $order_id,'gopay_order_id', $gopay_order_id );
            ...
            if( $price == 0 ){ $subs = false; }
            
            if ( $subs == true && wcs_order_contains_subscription( $order ) ) {
            //if ( $subs == true ) {
            ...
            update_post_meta( $order_id, '_paymentSessionId', esc_attr($paymentSessionId));
            ...
            update_post_meta($order_id,'order_encryptedSignature',$encryptedSignature);                              		
