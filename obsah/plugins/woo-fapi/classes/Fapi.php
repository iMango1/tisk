...
class FapiException extends Exception { }

/* Safety */

if (!function_exists('curl_init')) {
    throw new FapiException('Fapi lib needs the CURL PHP extension.');
}
if (!function_exists('json_decode')) {
    throw new FapiException('Fapi lib needs the JSON PHP extension.');
}
...
class Fapi {
    ...
    public function __construct($email, $password, $user_agent='PHPLib') {
        $this->email      = $email;    
    ...
    public function get_client_by_email($email) {
        return $this->get("/clients/search?email=$email");
    }
  
    public function get_clients($options = NULL) {
  ...
  public function download_invoice($id) {
    return $this->get("/invoices/$id./download.pdf");
  }
  ...
  private function run($path, $method, $data = NULL) {    
    ...
    if ($c === FALSE) {
      throw new FapiException('cURL failed to initialize.');
    }
    
    curl_setopt($c, CURLOPT_URL, "http://api.fapi.cz$path");
    curl_setopt($c, CURLOPT_RETURNTRANSFER, TRUE);
    curl_setopt($c, CURLOPT_FAILONERROR, FALSE); // to get error messages in response body
    curl_setopt($c, CURLOPT_USERPWD, "$this->email:$this->password");
    ...
    if ($response === FALSE) {
      throw new FapiException(sprintf("cURL failed with error #%d: %s", curl_errno($c), curl_error($c)), curl_errno($c));
    }
     
    if ($info['http_code'] >= 400) {
      throw new FapiException($response, $info['http_code']); 
    }
    curl_close($c);
